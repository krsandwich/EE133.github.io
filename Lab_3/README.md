...
## Abstract
Mixers are extremely pervasive in modern circuits, with applications ranging from radio recivers to phase detectors. The purpose of a mixer is to shift signals from one frequency span to another so that the signal can be transmitted or undergo additional procesing. In this lab, we present a hand-built double balanced passive RF Mixer. The architecture we chose was a FET ring mixer based on its popularity in modern wireless communication. We then evaluate our mixer based on commmon performance metrics including the noise figure, linearity, conversion loss and port isolation. 


## Background
 First, we will give some information passive double balanced switching RF mixer. Mixers are 3-port devices which take the sum and difference of two ports, RF and LO, and output both to the other port, IF. The mixer is passive in that it is not connected to a power source or configured in such a way as to provide gain. A double balanced mixer has full port isolation which allows it to increase linearity and supress more spurious products. We also used a switching mixer topology instead of a multiplier because we wanted to limit the noise penalty of our circuit. 
...
The PE4141 module represents the FET ring. Looking at the datasheet, each of its inputs is rated for frequencies ranging from 10 kHz to 1 GHz. Its made from GaAs so that it can be used in nuclear magnetic resonance/magnetic resonance imaging (MRI) applications. 
